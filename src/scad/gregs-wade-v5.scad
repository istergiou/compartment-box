...
// http://www.thingiverse.com/thing:548237 (e3d-v6)
include<inc/e3d_v6_all_metall_hotend.scad>
// http://www.thingiverse.com/thing:862716 (e3d-v6 volcano)
// TODO

// http://www.thingiverse.com/thing:625905
include<inc/parametric-fan.scad>
...
e3d_extrusion_mount=2048;

//default_extruder_mount=malcolm_extrusion_mount;
//default_extruder_mount=groovemount;
default_extruder_mount=e3d_extrusion_mount;
base_extra_depth=0; //2.80;

// xcarriage_mount type
prusa_xcarriage_mount=1;
einstain_xcarriage_mount=2;
default_xcarriage_mount=prusa_xcarriage_mount;
...
//%fan_mount_complete();
//translate([0,0,59]) rotate([90,0,0]) 
  wade(hotend_mount=default_extruder_mount,
	mounting_holes=default_mounting_holes);


//%translate([5,-45,-25/2]) rotate([60,0,0]) fan(30, 10,0);
//translate([-10,-35,0]) cube([40,40,2]);
// helper floor
//%translate([-100,-60,-20]) cube([200,1,600])
...
base_thickness=10;
base_length=70+2-6-26;
base_leadout=25+2+1-6-21+6; //25+2+1-6-21;
//base_extra_depth=0;
...
motor_mount_translation=[43.8,54,0];
...
layer_thickness=0.3200; //0.35;
filament_diameter=3.9;
...
idler_fulcrum_offset=608_diameter/2+3.5+m3_diameter/2;//+6;
...
module wade(
	...
	mounting_holes=default_mounting_holes,
	xcarriage_mount=default_xcarriage_mount)
{
	difference ()
	{
		union()
		{
			...
			translate([18,37,28]) 
				rotate([0,0,0]) difference() {
					cube([ 4,12,4]);
					translate([0,3,0]) cube([ 4,12-3*2,2]);				
				}
			// cable tie mount (bottom)
			translate([20,-5,28]) 
				difference() {
					cube([ 4,18,4]);
					#translate([0,3,0]) cube([ 4,18-3*2,2]);				
				}
			...
			// fan mount
			fanmount_z=8; // wade_block_depth+base_extra_depth-10;
			if(false) translate([-base_leadout-base_thickness,-base_thickness,0]) {
				difference() {
					union() {
						cylinder(r=base_thickness/2,h=fanmount_z,$fn=20);
						rotate([0,0,45]) translate([0,-base_thickness/2,0]) cube([base_thickness*1.5,10,fanmount_z], center=false);					
					}
			...
			// prusa_xcarriage_mount  
			if ( in_mask (xcarriage_mount,prusa_xcarriage_mount) ) {
				x=10;
				y=46; //26;
				z=wade_block_depth+base_extra_depth; //+6;
				translate([base_length-base_leadout-base_thickness/2,-y,0]) {
					difference() {
						union() {
							cube([x,y,z]);
							translate([18,18,17]) rotate([0,-90,0]){
								for(sx=[12,-12],sy=[18,-18])
			  						translate([+sx,sy,8]) 
			  							cylinder(r=3.5+1.5, h=10, $fn=30);
							}					
						}
						// Extruder mounting holes (screw holes and screw head)
						translate([18,18,17]) rotate([0,-90,0]){
		  					for(sx=[12,-12],sy=[18,-18]) translate([+sx,-sy,0]) cylinder(r=1.7, h=20, $fn=20);
		  					for(sx=[12,-12],sy=[18,-18]) translate([+sx,-sy,16]) cylinder(r=3.35, h=5, $fn=6);
							
						}					
					}
				}
			}
			// einstain_xcarriage_mount
			if ( in_mask (xcarriage_mount,einstain_xcarriage_mount) ) {
				x=10;
				y=48;
				z=wade_block_depth+base_extra_depth;
				translate([base_length-base_leadout-base_thickness/2,-y+4,0]) {
					difference() {
						union() {
							cube([x,y-4,z]);
							// einstain mount
						...
						// Extruder mounting holes (screw holes and screw head)
						translate([12,18,17]) rotate([0,-90,0]){
		  					for(sx=[12,-12],sy=[-18,18]) translate([+sx,-sy,0]) cylinder(r=1.7, h=20, $fn=20);
		  					for(sx=[12,-12],sy=[-18,18]) translate([+sx,-sy,10]) cylinder(r=3.5, h=20, $fn=5);
			...
			{
				rotate(-15)
				translate([-(idler_hinge_r+3),-idler_hinge_r-1.5,-wade_block_depth/2])
				difference()
				{
				cube([idler_hinge_r+3,
					idler_hinge_r*2+3,
					wade_block_depth/2 - idler_short_side/2+ idler_hinge_width+0.25+ layer_thickness]);
				translate([idler_hinge_r+2,(idler_hinge_r*2+3)/2,layer_thickness*3])
				 cylinder(r=idler_hinge_r+0.5,h=10,$fn=50);
				}
				rotate(-15)
					translate([-(idler_hinge_r+3),-idler_hinge_r-1.5,-idler_short_side/2+idler_hinge_width+0.25])
			...
			//translate([-base_leadout,-base_thickness/2,wade_block_depth+base_extra_depth]) cube([base_length,1,layer_thickness]);
		...
		rotate([-90,0,0])
		{
			...
			if (in_mask(hotend_mount,e3d_extrusion_mount))
				e3d_mount_holes(insulator_d=16);
		}
	}
}
...
module block_holes(mounting_holes=default_mounting_holes)
{
	...
	if (false) translate([-48.5+64+4,1,3]) {
		translate([-25,0,0]) { //-46
	...
	for (idle=[-1,1])
	{
		...
		rotate([0,90,0])
		{
			rotate([0,0,180/8])
			translate([0,0,-1])
			#cylinder(r=m4_diameter/2,h=wade_block_depth,$fn=8);	
			rotate([0,0,180/6])
...
module e3d_mount_holes(insulator_d=12.7)  
{  
	// http://wiki.e3d-online.com/images/1/12/DRAWING-V6-300-SINK-D.png
	echo("insulator_d",insulator_d);
	extruder_recess_d=insulator_d+1.3;
	extruder_recess_h=6+3.7;

	// the extruder
	%translate([0,0,extruder_recess_h+3]) rotate([0,180,180]) e3d();
	// Recess in base
	translate([0,0,-1])
	 cylinder(r=extruder_recess_d/2,h=extruder_recess_h+1);
	
	echo("wade_block_depth",wade_block_depth);
	echo("base_extra_depth",base_extra_depth);
	for (hole=[-1,1])
	 rotate(90,[1,0,0])
	 	// 3 is the distance of hole from base of extruder
	 	// 1.5 is half the diameter of the hole
	  translate([hole*(extruder_recess_d/2-1.5),3+1.5,-wade_block_depth/2-1])
	   	cylinder(r=1.5,h=wade_block_depth+2+base_extra_depth,$fn=10);
}

module wildseyed_mount_holes(insulator_d=16)
{  
	// http://wiki.e3d-online.com/images/1/12/DRAWING-V6-300-SINK-D.png
	echo("insulator_d",insulator_d);
	extruder_recess_d=insulator_d+0.7;
